...
export default function DistroResultsPanel({ distros, dict, lang }: DistroResultsPanelProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {distros.map((distro) => {
        // 🛡️ Salvavidas: si difficulty no viene de Supabase o cambia de nombre, evita el crash
        const difficultyKey = (distro.difficulty || 'beginner').toLowerCase();
          ...
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md ${
                  difficultyKey === 'intermediate' ? 'bg-warning/10 text-warning' : 
                  difficultyKey === 'advanced' ? 'bg-danger/10 text-danger' :
                  'bg-success/10 text-success' // Fallback para 'beginner' o cualquier valor indefinido
                }`}>
            ...
            <div className="border-t border-border pt-4 flex items-center justify-between text-[11px] text-muted-foreground font-medium">
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1"><Layers size={13} /> {distro.minRam || 0}GB</span>
                <span className="flex items-center gap-1"><Cpu size={13} /> {distro.minCpuCores || 0} CPU</span>
                <span className="flex items-center gap-1"><HardDrive size={13} /> {distro.minStorage || 0}GB</span>
              </div>
